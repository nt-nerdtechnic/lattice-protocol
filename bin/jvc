...
get_exclude_args() {
    ...
    # 2. 全局 .jvcignore (位於 skill-sync-manager)
...
usage() {
    ...
    echo "  fetch                - 掃描雲端 manifest 重建 JVC 地圖"
    echo "  pull                 - 執行同步 (自動處理依賴關係)"
    echo "  push <skill> <msg>   - 智慧推送 (Lock + Snapshot + Sync)"
    echo "  history <skill>      - 直接讀取雲端 JVC 歷史紀錄"
    echo "  status               - 比較本地與 JVC Master 差異"
    echo "  tree                 - 生成本地 SKILL_TREE.md"
    echo ""
...
fetch_master() {
    echo "🔄 JVC Fetch: 正在掃描雲端 manifest 節點..."
    ...
    echo "✅ Fetch 完成：偵測到 $(jq '.skills | length' "$MAP_FILE") 個有效節點。"
}
...
check_skill_status() {
    ...
    if [ -z "$cloud_info" ]; then echo "❓ JVC 找不到技能: [$s]"; return; fi
    ...
    if [[ "$l_ver" < "$m_ver" ]]; then echo "🔼 可更新：[$s] (v$l_ver -> v$m_ver)";
    elif [ "$m_ver" == "$l_ver" ] && [ "$diff_check" -gt 0 ]; then echo "🚨 內容衝突：[$s]";
    elif [[ "$l_ver" > "$m_ver" ]]; then echo "🆕 本地較新：[$s] (v$l_ver > v$m_ver)";
    else echo "✅ 最新：[$s] (v$l_ver)"; fi
}
...
pull_sync() {
    [ -z "$SYNC_TARGETS" ] && echo "🚨 錯誤：尚未設定同步目標" && exit 1
    echo "⏳ 正在啟動 JVC 同步 (處理依賴中)..."
...
push_skill() {
    local s=$1; local msg=$2; [ -z "$s" ] || [ -z "$msg" ] && echo "用法: push <skill> <msg>" && exit 1
    local cloud_info=$(get_cloud_info "$s")
    [ -z "$cloud_info" ] && echo "❌ 錯誤：JVC 找不到此技能" && exit 1
    local cloud_path=$(echo "$cloud_info" | jq -r ".cloud_path")
    local LOCK_DIR="$MASTER_REPO/$cloud_path/.lock"
    
    echo "🔒 獲取 JVC [$s] 鎖..."
    if ! mkdir "$LOCK_DIR" 2>/dev/null; then echo "🚨 鎖定失敗：該節點正被佔用。" && exit 1; fi
...
case "$1" in
    fetch) fetch_master ;;
    status) fetch_master > /dev/null; echo "--- JVC 同步狀態 ---";
        for t in $SYNC_TARGETS; do
            if [[ "$t" == cat:* ]]; then
                cat_name=${t#cat:}; cache="$LOCAL_REPO/skill-sync-manager/cloud_map_cache.json"
                SKILLS=$(jq -r ".skills | to_entries | .[] | select(.value.category == \"$cat_name\") | .key" "$cache")
    ...
    push) push_skill "$2" "$3" ;;
    history) info=$(get_cloud_info "$2"); [ -n "$info" ] && echo "$info" | jq -r '.history[]' || echo "無紀錄" ;;
    *) usage ;;
esac
